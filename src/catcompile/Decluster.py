...
from openquake.hmtk.parsers.catalogue import CsvCatalogueParser   # Reads an earthquake catalogue from CSV  # type: ignore
from openquake.hmtk.parsers.catalogue.csv_catalogue_parser import CsvCatalogueWriter  # Writes an earthquake catalogue to CSV  # type: ignore

# Plotting tools
# from openquake.hmtk.plotting.mapping import HMTKBaseMap
from openquake.hmtk.plotting.seismicity.catalogue_plots import plot_magnitude_time_density  # type: ignore
from openquake.hmtk.plotting.seismicity.occurrence.recurrence_plot import plot_recurrence_model  # type: ignore

# Seismicity tools: Events and declustering methods
from openquake.hmtk.seismicity.declusterer.dec_gardner_knopoff import GardnerKnopoffType1  # type: ignore
from openquake.hmtk.seismicity.declusterer.distance_time_windows import (GardnerKnopoffWindow,  # type: ignore
                                                                         GruenthalWindow, 
                                                                         UhrhammerWindow)

# Seismicity tools: Recurrence methods
from openquake.hazardlib.mfd import TruncatedGRMFD  # type: ignore
from openquake.hmtk.seismicity.occurrence.weichert import Weichert  # type: ignore
...
def using_HMT_catalogue(catalogue_filename,output_cat_dec):
    #Importing catalogue
    ...
    * cluster_flag = Vector indicating if the event is a foreshock (-1), mainshock (0) or aftershock (1)
    ...
    bval, sigma_b, aval, sigma_a = recurrence_estimator.calc(catalogue_dec,
                                                                recurrence_config,
...
def main():
    ...
    # Exploring & Processing Earthquake Catalogues
    catalogue_filename = f'outputs/1600-2024-merged-homogeneous_catalogue-filtered.csv'
    output_cat_dec = f'outputs/Overall_Seismicity_Dec_{formatted_date}.csv'
    using_HMT_catalogue(catalogue_filename,output_cat_dec)
